...
function needsScheduleRebuild(
    ...
    // Rebuild when slots were planned for a much longer exam than time left
...
export function useDishaProctorSnapshots({
    ...
    const captureAndUpload = useCallback(
        async (snapshotIndex: number) => {
            if (!attemptId || !packageId || capturingRef.current.has(snapshotIndex)) return;
        ...
        [attemptId, packageId, studentId, currentRoundNumber, serverCapturedIndexes]
    );
    ...
    const tick = useCallback(() => {
        ...
        // Also use elapsed time when student finishes quickly (wall-clock), not only short remaining window
